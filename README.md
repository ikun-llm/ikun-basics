<p align="center"><img src="https://raw.githubusercontent.com/ikun-llm/.github/main/profile/logo.png" width="120" /></p>
<h2 align="center">ikun-basics</h2>
<p align="center"><b>练习生的基础功课</b><br/><sub>Level 0 | 入门篇 — 零基础也能看懂</sub></p>

<p align="center">
  <img src="https://img.shields.io/badge/难度-小白友好-brightgreen?style=for-the-badge" />
  <img src="https://img.shields.io/badge/前置要求-会呼吸就行-blue?style=for-the-badge" />
</p>

---

> 在学唱跳 rap 篮球之前，你得先学会站起来。

本仓库用**最通俗的语言**讲解 AI/大模型背后的基础知识。没有复杂公式，没有论文黑话，只有让你"哦！原来是这样"的解释。

## 目录

| # | 章节 | 内容 | 一句话总结 | 图示预览 |
|---|------|------|-----------|---------|
| 1 | [大模型发展史](docs/01-history.md) | 从感知机到 ChatGPT | AI 怎么从"人工智障"变成"鸡你太美" | <img src="docs/images/01_history.png" width="200" /> |
| 2 | [CNN 和 RNN](docs/02-cnn-rnn.md) | 卷积神经网络 + 循环神经网络 | 一个看图，一个读文 | <img src="docs/images/02_cnn_rnn.png" width="200" /> |
| 3 | [Diffusion 扩散模型](docs/03-diffusion.md) | 从噪声中生成图片 | 从一团噪点里变出篮球 | <img src="docs/images/03_diffusion.png" width="200" /> |
| 4 | [Transformer 架构](docs/04-transformer.md) | 注意力机制 + 自注意力 | 大模型的心脏，ikun 的核心技术 | <img src="docs/images/04_transformer.png" width="200" /> |

## 阅读建议

- **完全零基础**：按顺序从 1 读到 4
- **有一点基础**：直接跳到第 4 章 Transformer
- **只想了解大概**：每章开头都有"一句话总结"，扫一遍就行

## 系列导航

| Level | Repo | 学什么 |
|-------|------|--------|
| **0** | **ikun-basics** ← 你在这里 | **AI 基础知识** |
| 1 | [ikun-tokenizer](https://github.com/ikun-llm/ikun-tokenizer) | 分词器原理 |
| 1 | [ikun-pretrain](https://github.com/ikun-llm/ikun-pretrain) | 从零预训练 |
| 1 | [ikun-2.5B](https://github.com/ikun-llm/ikun-2.5B) | SFT + LoRA 微调 |
| 2 | [ikun-DPO](https://github.com/ikun-llm/ikun-DPO) | 偏好对齐 |
| 2 | [ikun-GRPO](https://github.com/ikun-llm/ikun-GRPO) | 强化学习 |
| 2 | [ikun-Reason](https://github.com/ikun-llm/ikun-Reason) | 推理模型 |
| 3 | [ikun-MoE](https://github.com/ikun-llm/ikun-MoE) | 混合专家 |
| 3 | [ikun-Distill](https://github.com/ikun-llm/ikun-Distill) | 知识蒸馏 |
| 3 | [ikun-V](https://github.com/ikun-llm/ikun-V) | 多模态 |
| 4 | [ikun-deploy](https://github.com/ikun-llm/ikun-deploy) | 部署 |
